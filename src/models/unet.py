...
class UnetDecoder(nn.Module):
    ...
    def forward(self, *encoder_outputs):
        output = self.up1(encoder_outputs[-1])

        output = torch.cat([self.crop1(encoder_outputs[-2]), output], dim=1)
        output = self.decoder1(output)

        output = self.up2(output)
        output = torch.cat([self.crop2(encoder_outputs[-3]), output], dim=1)
        output = self.decoder2(output)

        output = self.up3(output)
        output = torch.cat([self.crop3(encoder_outputs[-4]), output], dim=1)
        output = self.decoder3(output)

        output = self.up4(output)
        output = torch.cat([self.crop4(encoder_outputs[-5]), output], dim=1)
